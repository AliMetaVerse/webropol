...
export class SMSYesNo extends BaseComponent {
    ...
    init() {
        ...
        if (!window.smsYesNoData) {
            window.smsYesNoData = (initialMode, qId, defaultText) => {
                return {
                    ...
                    init() {
                        ...
                        this.$watch('questionText', (value) => {
                            try {
                                ...
                                console.error('Failed to save SMS question text:', e);
                            }
                        });
                    },
    ...
    render() {
        const qNum         = this.getAttribute('question-number') || '1';
        const smsNum       = this.getAttribute('sms-number')      || '1';
        const characters   = this.getAttribute('characters')      || '0';
        const credits      = this.getAttribute('credits')         || '1';
        const questionText = this.getAttribute('question-text')   || 'Would you recommend Webropol to a colleague or friend? Please respond with: Y = Yes and N = No';
        const safeText     = questionText.replace(/'/g, "\\'");

        this.innerHTML = `
            <div x-data="smsYesNoData('${this.mode}', '${this.questionId}', '${safeText}')"
                ...
                <div x-show="selected && mode === 'edit'"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-2 bg-white px-1 py-1 rounded-lg border border-gray-100">
                            ...
                            <span class="text-xs font-medium text-gray-700">Selection</span>
                        </div>
                    </div>
                ...
                <div x-show="selected && mode === 'edit'" class="p-6 rounded-b-3xl bg-white">
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center gap-3">
                            <input type="radio" name="yn_${this.questionId}" class="w-4 h-4 text-webropol-primary-600 border-webropol-gray-400" disabled>
                            <span class="text-base">Y</span>
                        </label>
                        <label class="flex items-center gap-3">
                            <input type="radio" name="yn_${this.questionId}" class="w-4 h-4 text-webropol-primary-600 border-webropol-gray-400" disabled>
...
customElements.define('webropol-sms-yes-no', SMSYesNo);
